...
# get age
mean_age = infant_d %>% distinct(unique_id, age) %>% pull(age) %>% mean()
sd_age = infant_d %>% distinct(unique_id, age) %>% pull(age) %>% sd()

test_df = infant_d %>% filter(fam_or_test == 'test') %>% mutate(fam_duration = as.numeric(fam_duration), 
