...
\end{tabular}
\endgroup
\caption{Cross-validated RMSE across linking hypotheses and lesioned models. Mean RMSE is averaged across all parameter values, while best RMSE reflects the single best-performing parameter values.} 
\end{table}
...
\begin{figure}
\centering
\includegraphics{00_main_files/figure-latex/infant_exp1_res-1.pdf}
\caption{(\#fig:infant\_exp1\_res)Experiment 1, infant results}
\end{figure}

\begin{figure}
\centering
\includegraphics{00_main_files/figure-latex/adult_exp1_res-1.pdf}
\caption{(\#fig:adult\_exp1\_res)Experiment 1, adult results}
\end{figure}
...
Our exact replication via Children Helping Science showed similar results: We again found that animacy and number resulted in the strongest dishabituation in infants (animacy: \(\beta\) = 0.34; \emph{SE} = 0.14; \emph{t} = ; \emph{p} = 0.015, number: \(\beta\) = 0.35; \emph{SE} = 0.13; \emph{t} = 2.68; \emph{p} = 0.008), and pose violations did not elicit dishabituation (\(\beta\) = ; \emph{SE} = ; \emph{t} = ; \emph{p} = ). However, unlike the Zoom study, the identity violation did not elicit significant dishabituation in our Lookit study (\(\beta\) = 0.07; \emph{SE} = 0.14; \emph{t} = 0.46; \emph{p} = 0.646). \footnote{Note that we pre-registered the identity violation as a data quality check, since we had replicated that effect in both infant experiments. However, surprisingly, the identity condition was the only condition that differed from the Zoom experiment, so here we deviate from pre-registration and report these Lookit findings regardless given the high value of having a second sample of infant data.}

\begin{figure}
\centering
\includegraphics{00_main_files/figure-latex/infant_exp2_res-1.pdf}
\caption{(\#fig:infant\_exp2\_res)Experiment 2, infant results, combined across Zoom and Children Helping Science}
\end{figure}

\begin{figure}
\centering
\includegraphics{00_main_files/figure-latex/adult_exp2_res-1.pdf}
\caption{(\#fig:adult\_exp2\_res)Experiment 2, adult results}
\end{figure}

Next, we combined the two datasets (total n=123) to gain additional power. In an exploratory analysis, we ran the same linear model as on the individual datasets, since we found no additional variance explained by a random effect of dataset. We found again found significant dishabituation to animacy and number violations (number: \(\beta\) = 0.34; \emph{SE} = 0.11; \emph{t} = 3.19; \emph{p} = 0.002, number: \(\beta\) = 0.35; \emph{SE} = 0.1; \emph{t} = 3.37; \emph{p} = 0.001), marginally significant dishabituation to identity violations (\(\beta\) = 0.19; \emph{SE} = 0.11; \emph{t} = 1.77; \emph{p} = 0.078) and no dishabituation to the pose violation (\(\beta\) = 0.07; \emph{SE} = 0.1; \emph{t} = 0.66; \emph{p} = 0.513). Together, we found evidence that the magnitude of dishabituation was related to the similarity of the novel trial, as infants' looking time followed our expected ordering: animacy \textgreater{} number \textgreater{} identity \textgreater{} pose \textgreater{} background (Figure @ref(fig:infant\_exp2\_res)).

Next, we tested whether infants' dishabituation was better described by a ``same or different'' account, in which dishabituation responds similarly to any novel stimulus, or a ``graded'' account, in which the type of violation influences the magnitude of dishabituation. To do so, we conducted a model comparison in which violation type was either parsed into two levels (familiar or novel) or five levels (familiar, animacy, number, identity or pose). The model comparison significantly favored the more granular model (\(\chi\)(3) = 8.9; \emph{p} = 0.031), suggesting that infants were sensitive to the type of violation rather than just to whether the test stimulus was familiar or novel.

In adults, as in previous experiments, we found evidence for habituation (Figure @ref(fig:adult\_exp2\_res); STATS). More importantly, we also found evidence for graded dishabituation: the dishabituation magnitude to animacy violations was larger than to number violations (STATS) and to pose violations (STATS). The same pattern was found for identity violations (STATS). However, animacy violations were not different from identity type novelty, nor were number violations different from the pose violations (STATS). Qualitatively, we found that the ordering for dishabituation magnitude was animacy (STATS) \textgreater{} identity (STATS) \textgreater{} pose (STATS) \textgreater{} and number (STATS).
...
Using the previously best-fitting parameters in Experiment 1, we found that RANCH generalized to Experiment 2 successfully (Infants: STATS; Adults: STATS). Moreover, RANCH also showed a qualitative ordering of the dishabituation magnitude: animacy \textgreater{} identity \textgreater{} number \textgreater{} pose. This ordering was in line with the CNN-derived embedding distances between the different stimulus categories (Figure @ref(fig:embedding\_distances)), and similar to those seen in infants and adults.
...
To code the infants' looking time we used iCatcher+, a validated tool developed for robust and automatic annotation of infants' gaze direction from video \autocite{erel2022icatcher}. To obtain trial-wise looking time, we merged iCatcher+ annotations with trial timing information, thereby fully replacing manual coding of looking time. The correctness of iCatcher+ coding was manually supervised to check for failed face detections or distractions.
...
\begin{figure}
\centering
\includegraphics{00_main_files/figure-latex/goldilocks_explanation-1.pdf}
\caption{(\#fig:goldilocks\_explanation)Illustration of how predictions from the Goldilocks model in Fig. 1 were derived.}
\end{figure}

\hypertarget{parameter-robustness}{%
\subsection{Parameter robustness}\label{parameter-robustness}}

Some text about parameter robustness. Some text about parameter robustness. Some text about parameter robustness. Some text about parameter robustness. Some text about parameter robustness. Some text about parameter robustness. Some text about parameter robustness. Some text about parameter robustness. Some text about parameter robustness. Some text about parameter robustness. Some text about parameter robustness. Some text about parameter robustness. Some text about parameter robustness. Some text about parameter robustness. Some text about parameter robustness.Some text about parameter robustness. Some text about parameter robustness. Some text about parameter robustness. Some text about parameter robustness. Some text about parameter robustness.

\begin{figure}
\centering
\includegraphics{00_main_files/figure-latex/parameter_robustness-1.pdf}
\caption{(\#fig:parameter\_robustness)Histograms of RMSE for different linking hypotheses show that RANCH is largely robust to variations in parameters.}
\end{figure}
...
\caption{(\#fig:parameter\_interpretation)Histograms of model fit colored by priors. Results suggest that when the learner's noise is high, RANCH fits infant data better.}
\end{figure}
...
\hypertarget{control-experiment-for-experiment-2}{%
\subsection{Control experiment for Experiment 2}\label{control-experiment-for-experiment-2}}

Some text about control experiment. Some text about control experiment.Some text about control experiment. Some text about control experiment.Some text about control experiment. Some text about control experiment.Some text about control experiment. Some text about control experiment.Some text about control experiment. Some text about control experiment.Some text about control experiment. Some text about control experiment.Some text about control experiment. Some text about control experiment.Some text about control experiment. Some text about control experiment.Some text about control experiment. Some text about control experiment.Some text about control experiment. Some text about control experiment.
\includegraphics{00_main_files/figure-latex/control_exp-1.pdf}
