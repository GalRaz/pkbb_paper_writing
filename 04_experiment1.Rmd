...
```{r design, echo=FALSE, fig.width=20, fig.height=14, fig.cap="Experimental deisgn"}
...
CV_table = CV_table %>% mutate(`Mean RMSE` = c(mean(resnet50_EIG_CV$rmse), mean(resnet50_KL_CV$rmse), mean(resnet50_surprisal_short_CV$rmse), mean(resnet50_nolearning_CV$rmse), mean(resnet50_nonoise_CV$rmse))) %>% mutate(`Best RMSE` = c(min(resnet50_EIG_CV$rmse), min(resnet50_KL_CV$rmse), min(resnet50_surprisal_short_CV$rmse), min(resnet50_nolearning_CV$rmse), min(resnet50_nonoise_CV$rmse)))

table <- xtable(as.matrix(CV_table %>% as.data.frame()), caption = "Table 1: Crossvalidation results")
