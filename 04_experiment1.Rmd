...
# Number of subjects
total_n = infant_d %>% pull(unique_id) %>% unique() %>% length()

# number of subjects after exclusions
total_n_after_exclusion = infant_d %>% filter(!exclude) %>% pull(unique_id) %>% unique() %>% length()

# experiment-specific sample size
n_subject_expA = infant_d %>% filter(str_detect(unique_id, "A")) %>% unique() %>% nrow()
n_subject_expB = infant_d %>% filter(str_detect(unique_id, "B")) %>% unique() %>% nrow()
n_subject_expC = infant_d %>% filter(str_detect(unique_id, "C")) %>% unique() %>% nrow()

# age and sex
mean_age <- infant_d %>% group_by(unique_id) %>% summarize(age_in_months = mean(age)/30) %>% pull(age_in_months) %>% mean() %>% round(2)

age_sex_df = infant_d %>% distinct(unique_id, age, sex) %>% mutate(age_in_months = age/30) 
mean_age = age_sex_df %>% pull(age_in_months) %>% mean() %>% round(2)
sex = age_sex_df %>% count(sex)

# exclusions

complete_exclusions <- infant_d %>% distinct(unique_id, exclude) %>% 
...
# exclusions
excluded_df = infant_d %>% filter(!unique_id %in% complete_exclusions$unique_id)
