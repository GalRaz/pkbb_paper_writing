...
complete_exclusions <- exp1_infant_d %>% distinct(unique_id, exclude) %>% 
  group_by(unique_id) %>% filter(n() == 1, exclude) 

n_complete_exclusions = total_n - total_n_after_exclusion
...
# exclusions
excluded_df = exp1_infant_d %>% filter(!unique_id %in% complete_exclusions$unique_id)
