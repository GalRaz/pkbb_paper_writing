...
For the generalization, stimulus type experiment, we tested a combined sample of `r total_n` 7-10 month old infants, with `r n_subject_generalization` in the main experiment, `r n_subject_control` in the control experiment. `r n_complete_exclusions_stim_type` participants were excluded completely due to fussiness. An additional `r num_excluded_trials` individual test events were excluded (out of `r excluded_df %>% filter(fam_or_test == 'test') %>% nrow()`, `r round(100*num_excluded_trials/(excluded_df %>% filter(fam_or_test == 'test') %>% nrow()),1)`% of trials) did not make it into the final analysis.

Single trial exclusions occurred because 1) infants fussed out of the experiment at an earlier stage of the experiment, 2) infants looked at the stimuli for less than a total 2 seconds, 3) there were momentary external distractions in the home of the infant or 4) the gaze classifier performed so poorly that manual corrections were not viable. 
