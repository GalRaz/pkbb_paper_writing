...
In the exposure duration experiments, we presented infants with a series of animated animals, created using "Quirky Animals" assets from Unity (Fig. 1, [link to assets](https://tinyurl.com/469xxrn7)). The animals were walking, crawling or swimming, depending on the species. In a previous set studies, not included here (but see [OSF repository](https://osf.io/b3xsk/?view_only=5beeb6ed27874ae3bfe586ee2b38381a)), we showed infants a different stimulus set and failed to elicit replicable habituation, novelty or familiarity preferences. As a result, we modified the stimuli to be more appealing, and separated familiarization trials with curtains opening and closing to accentuate familiarization trials as separated occurrences of the stimulus.

In the stimulus type experiments, (Fig. 1, [link to assets](https://marketplace.unity.com/packages/3d/props/food/3d-prop-vegetables-and-fruits-237790)), to introduce animacy violations, we used the “3D Prop Vegetables and Fruits” (assets from Unity and animated bouncing and rotating trajectories. To create number violations, we duplicated the videos of single animal Unity assets and pasted them side-by-side.
...
In the exposure duration experiments, the deviant stimulus was always another animal. In the stimulus type experiments, one of four properties of the background animal could be violated: pose, identity, number or animacy. 

In the exposure duration experiments, each infant saw six blocks: Three different exposure durations (0, 4 and 8 in Exp. A, 1, 3 and 9 in Exp. B and 2, 4 and 6 in Exp. C) appeared twice each, once for each test event type (background or deviant). The longer exposure durations (8 or 9) were chosen based on our previous pilot studies with a different stimulus set ([OSF repository](https://osf.io/b3xsk/?view_only=5beeb6ed27874ae3bfe586ee2b38381a)), and the shorter durations were chosen to provide limited learning experience with the background. The order of blocks was counterbalanced between infants, and pairs of animals (background and deviant) were counterbalanced to be associated with each block type. Which animal was shown as the background and which as the deviant (if it was a deviant test trial) was randomized in each block.

In the stimulus type experiments, each infant saw four blocks within a session - the background trial was always one of the four test trials, and the remaining three test trials were picked randomly from the remaining four trial types (pose, identity, number, animacy).
